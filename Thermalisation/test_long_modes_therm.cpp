...
//			std::cout << "Calculating " << i << "\'th element of the ensemble..." << std::endl; }		
		...
		for (int n = 0; n < N_SAMPLE; n++) {	
			if (n%1000 == 0) { std::cout << "Taking " << n << "\'th sample..." << std::endl; }
			sol.ps(phi_k2, 0);
			sol.ps(chi_k2, 1);
			for (int j = 0; j < N_x; j++) {
	...
	for (auto& time_slice : power_spectrum_av_phi) {
		save_data(time_slice, "Out/Out_therm_Lang/therm_ps_phi", TEMP, ETA, 12);
	}	
	for (auto& time_slice : power_spectrum_av_chi) {
		save_data(time_slice, "Out/Out_therm_Lang/therm_ps_chi", TEMP, ETA, 12);
	}	
